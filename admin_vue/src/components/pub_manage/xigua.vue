  ...
  <div class="g-container xg-pub-manage">
                ...
                <div class="g-cfix">
                    <div style="display:inline-block;width:45px;margin-right:5px;" class="g-fl">
                        <div class="display:inline-block" style="width:40px;height:40px;border:1px solid #999;overflow:hidden;">
                            <img :src="scope.row.weixinthumb" :alt="scope.row.weixinname" style="dsiplay:inline-block;width:40px;height:40px;">
                        </div>
                    </div>
    ...
    color:#fff;font-size:12px;padding: 0px 4px;line-height:18px;">授权</span>
      ...
      <el-table-column label="性别比例" width="">
        <template slot-scope="scope">
          <div>男 {{scope.row.w_sexprop}}%</div>
          <div>女 {{scope.row.w_sexprop}}%</div>
        </template>
      </el-table-column>

      <el-table-column label="重要指标" width="">
        <template slot-scope="scope">
          <div>平均阅读数	{{scope.row.send_read}}</div>
          <div>首条平均阅读数	{{scope.row.first_read}}</div>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="sale_id" label="跟进渠道" width=""></el-table-column>
  ...
  export default {
    ...
    methods: {
        getList() {
            ...
            // ajax
            this.$http.get('/Data/xg_mps.html', {
                params: requestData
            })
            .then(response => {
